...
require_once "/var/www/html/seguimientopedidos-dev/Phpexcel/Classes/PHPExcel/IOFactory.php";
require_once "/var/www/html/seguimientopedidos-dev/Phpexcel/Classes/PHPExcel.php";
...
//Incia Servicios para consumir
	...
	private function loginUser() {
		...
		if (!empty($usuarioid) and !empty($password)) {
			...
			$sqlLogin = ("SELECT id, login, nombre, identificacion, perfil, password
							FROM seguimientopedidos.usuarios
...
/********************************/
	...
	// 	$id = $params['id'];
	//     $datos = $params['datos'];
	//     $nuevopwd = md5($datos['pwdNuevaConfirm']);
	//     $pwdAntDigUser = md5($datos['pwdActual']);
	    	
	//     $sqlUsuario = "select password from seguimientopedidos.usuarios where id='$id'";
	//     $rst = $this->connseguimiento->query($sqlUsuario);
	//     $pwdAntEnBD = $rst->fetch_row();

	// 	if($pwdAntDigUser == $pwdAntEnBD[0]){
	// 		$sqlUsuario="update seguimientopedidos.usuarios set password='$nuevopwd' where id='$id'"; 
	// 		$rst = $this->connseguimiento->query($sqlUsuario);
	// 		$this->response($this->json('Contraseña actualizada'), 201);
	// 	}else{
	...
	private function buscarPedidoSegui() {
		...
		if ($rst->num_rows > 0 || $remite == "Gestión contact center") {
			...
			$sqlpedidocontingencia = ("	SELECT * FROM seguimientopedidos.contingencias
			...
			if ($rstContingencia->num_rows > 0) {
				...
				$sqlpedidoproducto = ("	SELECT * FROM seguimientopedidos.contingencias
	...
	private function editarUsuario() {
		...
		$password = $login['PASSWORD'];
		//echo var_dump($login);
		$sqlUsuario = "update seguimientopedidos.usuarios set " .
	...
	private function editarTecnico() {
		...
		$sqlUsuario = "update seguimientopedidos.tecnicos set " .
			"nombre='$nombre',identificacion='$identificacion', " .
	...
	private function editAlarma() {
		...
		$sqlUsuario = "update seguimientopedidos.alarmas set " .
	...
	private function creaTecnico() {
		...
		$sql = " INSERT INTO seguimientopedidos.tecnicos ( " .
			" IDENTIFICACION, " .
			" NOMBRE, " .
			" CIUDAD, " .
			" CELULAR, " .
	...
	private function creaUsuario() {
		...
		$sql = " INSERT INTO seguimientopedidos.usuarios ( " .
			" IDENTIFICACION, " .
			" NOMBRE, " .
			" LOGIN, " .
			" PERFIL, " .
	...
	private function listadoUsuarios() {
		...
		$query = "SELECT a.ID, " .
			...
			"  a.IDENTIFICACION, " .
			"  a.LOGIN, a.perfil, " .
			"  (select b.nombre from seguimientopedidos.perfiles b where b.perfil=a.perfil) as PERFIL, " .
			"  a.PASSWORD " .
			" FROM seguimientopedidos.usuarios a" .
			"	where 1=1 " .
			" 	$parametro order by a.nombre ASC " .
			" limit 100 offset $pagina ";

		$queryCount = " select count(*) as Cantidad from seguimientopedidos.usuarios h " .
			" where 1=1 " .
			" $parametro ";
		//echo $query;
	...
	private function seguimientoClick() {
		...
		$query = "SELECT fecha, visita_atendido, sube_no_visita, no_sube, reagendado, " .
			...
			"FROM seguimientopedidos.seguimiento_Click_resumen " .
	...
	private function listadoAlarmas() {
		...
		$query = "SELECT * " .
			"FROM seguimientopedidos.alarmas";
		//echo $query;
	...
	private function nuevaAlarma() {
		...
		$sql = " INSERT INTO seguimientopedidos.alarmas ( " .
			" cantidad_campos, " .
			" ciudad, " .
			" nombre_alarma, " .
			" mensaje, " .
	...
	private function resumencontingencias() {
		...
		$query = "select logindepacho, pedido, horagestion, logincontingencia, horacontingencia, " .
			...
			"AND accion IN ('Cambio de equipo', 'Contingencia', 'Refresh', 'Registros ToIP') " .
			"and pedido <> '' " .
		...
		$queryestadosMes = ("SELECT (CASE WHEN acepta IS NULL THEN 'Pendiente' ELSE acepta END) estado, COUNT(*) total,
									(SELECT COUNT(*)
										FROM contingencias C2
										WHERE horagestion BETWEEN ('$diaInicial 00:00:00') AND ('$diaFinal 23:59:59')
										AND C2.accion IN ('Cambio de equipo', 'Contingencia', 'Refresh', 'Registros ToIP')
									) totalestados
								FROM contingencias AS C1
								WHERE horagestion BETWEEN ('$diaInicial 00:00:00') AND ('$diaFinal 23:59:59')
								AND pedido <> ''
								AND C1.accion IN ('Cambio de equipo', 'Contingencia', 'Refresh', 'Registros ToIP')
								GROUP BY estado
								ORDER BY total DESC
							");
	...
	private function listadoTecnicos() {
		...
		$query = "select a.ID, a.IDENTIFICACION, a.NOMBRE, a.CIUDAD, a.CELULAR,  a.empresa, " .
			" (select b.nombre from seguimientopedidos.empresas b where b.id=a.empresa) as NOM_EMPRESA " .
			" from seguimientopedidos.tecnicos a " .
			" where 1=1 $parametro order by a.nombre ASC " .
			" limit 100 offset $pagina ";

		$queryCount = " select count(*) as Cantidad from seguimientopedidos.tecnicos h " .
			" where 1=1 " .
			" $parametro ";
		//echo $query;
		$rr = $this->connseguimiento->query($queryCount);
	...
	private function ResumenInsta() {
		...
		$query = "SELECT DEPARTAMENTO_OPERACION, count(pedido_id) total_pedidos, sum(productos) total_productos " .
			"FROM seguimientopedidos.pendi_insta " .
			"where fecha = '$today' " .
		...
		$RESPONSABLE_GESTION = "SELECT RESPONSABLE_GESTION, sum(total) totaldireccion, sum(Entre_0_5) Entre_0_5, sum(Entre_5_10) Entre_5_10, " .
			...
			"FROM seguimientopedidos.resumenpendientesInstaresponsable " .
			"$condition  " .
		...
		$TIPO_SOLICITUD_ORIG = "SELECT solicitud, sum(total) totalsolicitud, sum(Entre_0_5) Entre_0_5, sum(Entre_5_10) Entre_5_10, " .
			"sum(Entre_10_15) Entre_10_15, sum(Entre_15_30) Entre_15_30, sum(Mayor_30) Mayor_30 " .
			"FROM seguimientopedidos.resumenpendientesInstasolicitudRango " .
			"$condition  " .
		...
		$NOVEDADES = "SELECT solicitud, sum(total) total, sum(sinAgenda) Sin_Agenda, sum(vencida) Vencida, " .
			...
			"FROM seguimientopedidos.resumenpendientesInstaSolicitudAgenda " .
			"$condition " .
			"group by solicitud order by sum(total) DESC ";

		$historico_rangos = "SELECT * FROM seguimientopedidos.HistoricoPendientesInsta " .
			"where fecha BETWEEN " .
		...
		$historico_porcentajes = "SELECT ROUND((Entre_0_5/total)*100, 2) porecentaje0_5, " .
			...
			"FROM seguimientopedidos.HistoricoPendientesInsta  " .
			"where fecha = '$today' ";
	...
	private function borrarUsuario() {
		...
		$sql = "delete from seguimientopedidos.usuarios where id=$id ";
	...
	private function borrarAlarma() {
		...
		$id = $params['id'];
		//echo 	$id;
		$sql = "delete from seguimientopedidos.alarmas where id=$id ";
	...
	private function borrarTecnico() {
		...
		$params = json_decode(file_get_contents('php://input'), true);
		$id = $params['id'];
		$sql = "delete from seguimientopedidos.tecnicos where id=$id ";
	...
	private function deleteregistrosCarga() {
		...
		$sql = "delete from seguimientopedidos.carga_archivos where id='$id' ";
	...
	private function ciudades() {
		...
		$query = " SELECT DISTINCT DEPARTAMENTO, CIUDAD " .
			" FROM seguimientopedidos.ciudades " .
			" ORDER BY CIUDAD ASC ";
	...
	private function procesos() {
		...
		$query = " SELECT DISTINCT trim(PROCESO) proceso" .
			" FROM seguimientopedidos.procesos " .
	...
	private function gestionComercial() {
		...
		$query = " SELECT DISTINCT GESTION" .
			" FROM seguimientopedidos.procesos_comercial " .
	...
	private function buscaregistros() {
		...
		$sqlnovedades = "SELECT DISTINCT NOVEDAD  " .
			"FROM seguimientopedidos.gestor_historicos_reagendamiento " .
	...
	private function novedadesTecnico() {
		...
		$query = ("	SELECT id, cedulaTecnico, nombreTecnico, contracto, proceso, pedido, tiponovedad, /*region,*/ municipio, situacion, horamarcaensitio,/*detalle, */observaciones, idllamada
	...
	private function guardarNovedadesTecnico() {
		...
		$situaciontriangulo = utf8_decode($datos['situaciontriangulo']);
		...
		if ($tiponovedad == 'Comportamiento Tecnicos' and $cedulaTecnico == null) {

					$sqlInsetar = ("
							INSERT INTO NovedadesVisitas
								(fecha, usuario, tiponovedad, pedido, contracto, cedulaTecnico, nombreTecnico, proceso, region, municipio, situacion, detalle, observaciones, idllamada)
							VALUES
								(NOW(), '$login', '$tiponovedad', '$pedido', UPPER('$contrato2'), TRIM($cedulaTecnico2), TRIM('$nombreTecnico2'), TRIM('$proceso2'), LOWER('$region'), LOWER('$municipio2'), LOWER('$situacion'), LOWER('$detalle'), LOWER(TRIM('$observaciones')), TRIM('$idllamada'))
						");
		...
		} else if ($tiponovedad == 'Comportamiento Tecnicos' and $region <> null) {

					$sqlInsetar = ("
							INSERT INTO NovedadesVisitas
								(fecha, usuario, tiponovedad, pedido, contracto, cedulaTecnico, nombreTecnico, proceso, region, municipio, situacion, detalle, observaciones, idllamada)
							VALUES
								(NOW(), '$login', '$tiponovedad', '$pedido', '$contracto', TRIM($cedulaTecnico), UPPER(TRIM('$nombreTecnico')), TRIM('$proceso'), LOWER('$region'), LOWER('$municipio'), LOWER('$situacion'), LOWER('$detalle'), LOWER(TRIM('$observaciones')), TRIM('$idllamada'))
						"); 
					...
					$sqlInsetar = ("
							INSERT INTO NovedadesVisitas
								(fecha, usuario, tiponovedad, pedido, contracto, cedulaTecnico, nombreTecnico, proceso, region, municipio, situacion, horamarcaensitio, observaciones, idllamada)
							VALUES
								(NOW(), '$login', '$tiponovedad', '$pedido', UPPER('$contrato2'), TRIM($cedulaTecnico2), TRIM('$nombreTecnico2'), LOWER('$proceso2'),LOWER('$region'), LOWER('$municipio2'), LOWER('$situaciontriangulo'), '$horamarcasitio', LOWER(TRIM('$observaciones')), TRIM('$idllamada'))
						"); 
					...
					$sqlInsetar = ("
							INSERT INTO NovedadesVisitas
								(fecha, usuario, tiponovedad, pedido, contracto, cedulaTecnico, nombreTecnico, proceso, region, municipio, situacion, horamarcaensitio, observaciones, idllamada)
							VALUES
								(NOW(), '$login', '$tiponovedad', '$pedido', '$contracto', TRIM($cedulaTecnico), UPPER(TRIM('$nombreTecnico')), TRIM('$proceso'), LOWER('$region'), LOWER('$municipio'), LOWER('$situaciontriangulo'), '$horamarcasitio', LOWER(TRIM('$observaciones')), TRIM('$idllamada'))
						"); 
	...
	private function csvNovedadesTecnico() {
		...
		$query = ("	SELECT n.fecha, n.usuario, n.municipio, n.region, n.proceso, n.horamarcaensitio, n.tiponovedad, n.pedido, n.cedulaTecnico, n.nombreTecnico, n.contracto, n.situacion, n.detalle, n.observaciones, n.idllamada
	...
	private function gestionEscalimiento() {
		...
		$query = "SELECT g.gestion FROM seguimientopedidos.Gestiones g";
	...
	private function observacionEscalimiento() {
		...
		$query = " 	SELECT o.observacion FROM seguimientopedidos.Gestiones g
					INNER JOIN seguimientopedidos.GestionesObservaciones go 	ON g.codGestion=go.codGestion
					INNER JOIN seguimientopedidos.Observaciones o				ON go.codObservacion=o.codObservacion
	...
	private function notasEscalamiento() {
		...
		$query = " 	SELECT n.nota FROM seguimientopedidos.Gestiones g
							INNER JOIN seguimientopedidos.GestionesObservaciones go 	ON g.codGestion=go.codGestion
							INNER JOIN seguimientopedidos.Observaciones o				ON go.codObservacion=o.codObservacion
							INNER JOIN seguimientopedidos.ObservacionesNotas ot			ON o.codObservacion=ot.codObservacion
							INNER JOIN seguimientopedidos.Notas n						ON ot.codNota=n.codNota
	...
	private function infoEscalamiento() {
		...
		if (isset($datos['id'])) {
			...
			$sqlUpdate = ("
				UPDATE seguimientopedidos.Escalamientos e
	...
	private function csvEscalamientoExp() {
		...
		$query = ("	SELECT 	e.fechaGestion, e.pedido, e.tecnico, e.loginDespachador, e.nombreDespachador, e.tecnologia, e.region, e.ciudad, e.grupoCola, e.gestion,
						...
						FROM seguimientopedidos.Escalamientos e
						WHERE 1=1
						AND e.fechaGestion BETWEEN ('$fechaini 00:00:00') AND ('$fechafin 23:59:59')
				");

		$queryCount = ("	SELECT COUNT(pedido) AS Cantidad FROM seguimientopedidos.Escalamientos
	...
	private function csvVisitasEnConjuntoExp() {
		...
		$query = ("	SELECT 	v.pedido, v.tecnicopremisas, v.tecnicoinfraestructura, v.fechavisita, v.region, v.municipio, v.contrato, v.gestion, v.quiensolicitavisita, v.notas, v.grupo, v.fechasolicitud, v.fechafingestion
						FROM seguimientopedidos.visitasenconjunto v
						WHERE 1=1
						AND v.fechasolicitud BETWEEN ('$fechaini 00:00:00') AND ('$fechafin 23:59:59')
				");

		$queryCount = ("	SELECT COUNT(pedido) AS Cantidad FROM seguimientopedidos.visitasenconjunto
	...
	private function registrosComercial() {
		...
		$query = "SELECT a.ID, " .
			...
			" FROM seguimientopedidos.registros_comercial a " .
			"	where 1=1 " .
			" 	$parametro " .
		...
		$queryCount = " select count(*) as Cantidad from seguimientopedidos.registros_comercial h " .
			" where 1=1 " .
			" $parametro ";
		//echo $queryCount;
		$rr = $this->connseguimiento->query($queryCount);
	...
	private function CrearpedidoComercial() {
		...
		$sql = " INSERT INTO seguimientopedidos.registros_comercial ( " .
			" LOGIN_ASESOR, " .
			" PEDIDO_ACTUAL, " .
			" PEDIDO_NUEVO, " .
			" CIUDAD, " .
	...
	private function Accionesoffline() {
		...
		$query = " SELECT DISTINCT ACCION" .
			" FROM seguimientopedidos.accionesoffline " .
	...
	private function CrearpedidoOffline() {
		...
		$sql = " INSERT INTO seguimientopedidos.registros_offline ( " .
			" LOGIN_ASESOR_OFF, " .
			" LOGIN_ASESOR, " .
			" PEDIDO, " .
			" PROCESO, " .
	...
	private function ingresarPedidoAsesor() {
		...
		if ($datosClick['pEDIDO_UNE'] == "" || $datosClick['pEDIDO_UNE'] == "TIMEOUT") {
			...
			$producto = $datosClick['uNETecnologias'] . "-" . $datosClick['uNEProductos'];
		...
		if ($proceso == 'Reparaciones') {
			$sql1 = "INSERT INTO seguimientopedidos.registros (pedido, id_tecnico, empresa, asesor, observaciones, " .
				"accion,tipo_pendiente,proceso,producto,duracion,llamada_id,prueba_integrada,codigo_familiar, " .
				"smartplay,toip,inter,iptv,telev,totdm,plantilla,despacho,id_cambio_equipo) VALUES ('$pedido', '$tecnico', '$nombre_de_la_empresa', " .
				...
				"'$iptv','$telev_hfc','$telefonia_tdm','$plantilla','$despacho','$idcambioequipo')";
		} else {
			$sql1 = "INSERT INTO seguimientopedidos.registros (pedido, id_tecnico, empresa, asesor, observaciones, " .
				"accion,tipo_pendiente,proceso,producto,duracion,llamada_id,plantilla,despacho)  " .
				"VALUES ('$pedido', '$tecnico', '$nombre_de_la_empresa', upper('$user'), '$observaciones', " .
				"'$accion','$subaccion','$proceso','$producto','$duracion_llamada','$id_llamada','$plantilla','$despacho')";
		}
	...
	private function registrosOffline() {
		...
		$query = "SELECT LOGIN_ASESOR_OFF, " .
			...
			"FROM seguimientopedidos.registros_offline";
		//echo $query;

		$queryCount = " select count(*) as Cantidad from seguimientopedidos.registros_offline h " .
			" where 1=1 ";
		//echo $queryCount;
		$rr = $this->connseguimiento->query($queryCount);
		$counter = 0;
	...
	private function cargaRegistros() {
		...
		//   $truncate="TRUNCATE TABLE seguimientopedidos.nps";
		...
		for ($v = $start_v; $v <= $end_v; $v++) {
			...
			for ($h = $start_h; ord($h) <= ord($end_h); $this->pp($h)) {
				...
				if ($h == "D") {
					$LOGIN_ASESOR = utf8_decode($cellValue);
				}
				if ($h == "E") {
				...
				if ($h == "I") {
					$FECHA = \PHPExcel_Style_NumberFormat::toFormattedString($cellValue, 'YYYY-MM-DD h:mm:ss');
				}
				if ($h == "J") {
	...
	private function cargar_datos() {
		...
		for ($v = $start_v; $v <= $end_v; $v++) {
			...
			if ($tipo_carga == 'vistaCliente') {
				$sql = "insert into seguimientopedidos.carga_agenda (pedido_id, cliente, " .
	...
	private function getRegistrosCarga() {
		...
		$query = "select a.id, a.nombre_archivo, a.tipo, a.fecha_carga, a.login,(select  " .
			...
			"from seguimientopedidos.alarmados c where a.id=c.id_archivo) " .
			"else (select count(b.pedido_id) " .
			"from seguimientopedidos.carga_agenda b where a.id=b.archivo_id) " .
			"END) TOTAL_REGISTROS " .
			"FROM seguimientopedidos.carga_archivos a order by fecha_carga DESC limit 10";
	...
	private function graficaDepartamento() {
		...
		if ($mesenviado == "" || $mesenviado == undefined) {
			...
			$query = "select max(fecha_instalacion) fecha from seguimientopedidos.nps ";
		...
		$query = "select gen.regional, round(((select count(respuesta)  " .
			"from seguimientopedidos.nps where num_respuesta = '5' and num_pregunta = '4' and mes = gen.mes and regional = gen.regional) " .
			"-(select count(respuesta) " .
			"from seguimientopedidos.nps where num_respuesta = '1' and num_pregunta = '4' and mes = gen.mes and regional = gen.regional) " .
			"-(select count(respuesta) " .
			"from seguimientopedidos.nps where num_respuesta = '2' and num_pregunta = '4' and mes = gen.mes and regional = gen.regional) " .
			"-(select count(respuesta) " .
			"from seguimientopedidos.nps where num_respuesta = '3' and num_pregunta = '4' and mes = gen.mes and regional = gen.regional))/ " .
			"(select count(respuesta) " .
			"from seguimientopedidos.nps where num_pregunta = '4' and mes = gen.mes and regional = gen.regional)*100,2) as NPS " .
			"from seguimientopedidos.nps gen  " .
			"where mes = '$nom_mes'  " .
	...
	private function DepartamentosContratos() {
		...
		$sql = "select gen.regional, round((select count(num_respuesta) " .
			"from seguimientopedidos.nps  " .
			"where num_respuesta = '5' and num_pregunta = '4' and contratista = 'EIA' " .
			"and regional = gen.regional and mes=gen.mes)/ " .
			"(select count(num_respuesta) from seguimientopedidos.nps where num_respuesta = '5' and num_pregunta = '4'  " .
			"and regional = gen.regional and mes=gen.mes)*100, 2)as EIA,  " .
			"round((select count(num_respuesta) from seguimientopedidos.nps " .
			...
			"from seguimientopedidos.nps gen where  contratista = gen.contratista " .
			"and mes = '$nom_mes' " .
		...
		if ($dptoContrato->num_rows > 0) {
			...
			$query = "select gen.contratista contratista, " .
				...
				"from seguimientopedidos.nps where contratista = gen.contratista and num_pregunta = '4'  " .
	...
	private function graficaAcumulados() {
		...
		$query = "select gen.respuesta, count(gen.respuesta) total, " .
			...
			"from seguimientopedidos.nps where num_pregunta = '$pregunta' and mes = gen.mes limit 1 )) *100, 2) as porcentaje " .
			"from seguimientopedidos.nps gen  " .
		...
		$Sqlmeses = "select distinct mes from seguimientopedidos.nps";
			...
			while ($row = $rstMeses->fetch_assoc()) {
				...
				if ($pregunta == "2") {
					$SqlAcumulado = "select mes, round(((select count(respuesta) " .
						"from seguimientopedidos.nps where num_pregunta = '2' and num_respuesta = '5' " .
						"and mes = '$nom_mes')+ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '2' and num_respuesta = '4' " .
						"and mes = '$nom_mes')- " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '2' and num_respuesta = '1'  " .
						"and mes = '$nom_mes'))/ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '2' and mes = '$nom_mes' " .
						")*100, 2) as NPS " .
						"from seguimientopedidos.nps " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
					...
					$SqlAcumulado = "select mes, round(((select count(respuesta) " .
						"from seguimientopedidos.nps where num_pregunta = '3' and num_respuesta = '1' " .
						"and mes = '$nom_mes')+ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '3' and num_respuesta = '2' " .
						"and mes = '$nom_mes')- " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '3' and num_respuesta = '5'  " .
						"and mes = '$nom_mes'))/ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '3' and mes = '$nom_mes' " .
						")*100, 2) as NPS " .
						"from seguimientopedidos.nps " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
					...
					$SqlAcumulado = "select mes, round(((select count(num_respuesta) " .
						"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '5' " .
						"and mes = '$nom_mes')-(select count(num_respuesta) " .
						"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '3'  " .
						"and mes = '$nom_mes')-(select count(num_respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '2'  " .
						"and mes = '$nom_mes')-(select count(num_respuesta)  " .
						"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '1'  " .
						"and mes = '$nom_mes'))/(select count(num_respuesta)   " .
						"from seguimientopedidos.nps where num_pregunta = '$pregunta' and mes = '$nom_mes' " .
						")*100, 2)  as NPS " .
						"from seguimientopedidos.nps " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
	...
	private function graficaAcumuladosrepa() {
		...
		if ($mesenviado == "" || $mesenviado == undefined) {
			...
			$query = "select max(FECHA_2) fecha from seguimientopedidos.npsreparaciones ";
		...
		$query = "select gen.respuesta, count(gen.respuesta) total, " .
			...
			"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and mes = gen.mes limit 1 )) *100, 2) as porcentaje " .
			"from seguimientopedidos.npsreparaciones gen  " .
			"where gen.num_pregunta = '$pregunta'  " .
			"and mes = '$nom_mes' " .
			"group by gen.respuesta ";
		...
		$Sqlmeses = "select distinct mes from seguimientopedidos.npsreparaciones";
		...
		if ($rstMeses->num_rows > 0) {
			...
			while ($row = $rstMeses->fetch_assoc()) {
				...
				if ($pregunta == "2") {
					$SqlAcumulado = "select mes, round(((select count(respuesta) " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '2' and num_respuesta = '5' " .
						"and mes = '$nom_mes')+ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '2' and num_respuesta = '4' " .
						"and mes = '$nom_mes')- " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '2' and num_respuesta = '1'  " .
						"and mes = '$nom_mes'))/ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '2' and mes = '$nom_mes' " .
						")*100, 2) as NPS " .
						"from seguimientopedidos.npsreparaciones " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
				} else if ($pregunta == "3") {
					$SqlAcumulado = "select mes, round(((select count(respuesta) " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '3' and num_respuesta = '1' " .
						"and mes = '$nom_mes')+ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '3' and num_respuesta = '2' " .
						"and mes = '$nom_mes')- " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '3' and num_respuesta = '5'  " .
						"and mes = '$nom_mes'))/ " .
						"(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '3' and mes = '$nom_mes' " .
						")*100, 2) as NPS " .
						"from seguimientopedidos.npsreparaciones " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
					...
					$SqlAcumulado = "select mes, round(((select count(respuesta) " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and num_respuesta = '5' " .
						"and mes = '$nom_mes')-(select count(respuesta) " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and num_respuesta = '3'  " .
						"and mes = '$nom_mes')-(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and num_respuesta = '2'  " .
						"and mes = '$nom_mes')-(select count(respuesta)  " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and num_respuesta = '1'  " .
						"and mes = '$nom_mes'))/(select count(respuesta)   " .
						"from seguimientopedidos.npsreparaciones where num_pregunta = '$pregunta' and mes = '$nom_mes' " .
						")*100, 2)  as NPS " .
						"from seguimientopedidos.npsreparaciones " .
						"where mes = '$nom_mes' " .
						"group by  mes ";
	...
	private function DemePedidoEncuesta() {
		...
		$query = "select idnps, telefono, cedula, detalle, fecha_instalacion, departamento, municipio, contratista, Interfaz, respuesta, semana " .
			"from seguimientopedidos.nps " .
			"where semana = (select max(semana) from seguimientopedidos.nps) " .
			"and num_pregunta = '4' " .
		...
		if ($rst->num_rows > 0) {
			...
			while ($row = $rst->fetch_assoc()) {
				...
				$query = "UPDATE seguimientopedidos.nps SET " .
	...
	private function resumenSemanas() {
		...
		if ($pregunta == "1" || $pregunta == "6" || $pregunta == "7" || $pregunta == "8") {
			...
			$query = "round((select count(num_respuesta) " .
				...
				"from seguimientopedidos.nps where semana = gen.semana and mes = gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as SI,   " .
				"round((select count(num_respuesta)   " .
				"from seguimientopedidos.nps   " .
				"where num_respuesta = '2' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)/  " .
				"(select count(num_pregunta)   " .
				"from seguimientopedidos.nps where semana = gen.semana and mes = gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NO ";
			...
			$query = "round((select count(num_respuesta) " .
				...
				"        where num_respuesta = '2' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)/ " .
				"        (select count(num_pregunta)  " .
				"from seguimientopedidos.nps where semana = gen.semana and mes = gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBNO, " .
				"round((select count(num_respuesta)  " .
				"		from seguimientopedidos.nps  " .
				"        where num_respuesta = '3' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)/ " .
				"        (select count(num_pregunta)  " .
				"from seguimientopedidos.nps where semana = gen.semana and mes = gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NOSEGURO, " .
				"round((select count(num_respuesta)  " .
				"		from seguimientopedidos.nps " .
				"        where num_respuesta = '4' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)/ " .
				"        (select count(num_pregunta)  " .
				"from seguimientopedidos.nps where semana = gen.semana and mes = gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBSI, " .
				"round((select count(num_respuesta)  " .
				"		from seguimientopedidos.nps " .
				...
				"from seguimientopedidos.nps where num_respuesta = '5' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_respuesta = '3' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_respuesta = '2' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana)-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_respuesta = '1' and num_pregunta = '$pregunta' and mes = gen.mes and semana = gen.semana))/(select count(respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and semana = gen.semana)*100, 2) as NPS ";
		...
		if ($rst->num_rows > 0) {
			...
			$sql1 = "select round(((select count(num_respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '5' and mes = '$nom_mes')-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '3' and mes = '$nom_mes')-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '2' and mes = '$nom_mes')-(select count(respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and num_respuesta = '1' and mes = '$nom_mes'))/(select count(respuesta) " .
				"from seguimientopedidos.nps where num_pregunta = '$pregunta' and mes = '$nom_mes')*100, 2) as NPS ";
			$rr = $this->connseguimiento->query($sql1);
			...
			if ($pregunta == "1" || $pregunta == "6" || $pregunta == "7" || $pregunta == "8") {
				...
				$querydiario = "round((select count(num_respuesta) " .
					...
					"(select count(num_pregunta) " .
					"from seguimientopedidos.nps where num_pregunta = '$pregunta'  and mes=gen.mes " .
			...
			if ($pregunta == "1" || $pregunta == "6" || $pregunta == "7" || $pregunta == "8") {
				...
				$querydepartamento = "round((select count(num_respuesta)  " .
					...
					"(select count(num_pregunta)  " .
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta'  limit 1 )*100, 2) as SI, " .
					"round((select count(num_respuesta) " .
					"from seguimientopedidos.nps  " .
					"where num_respuesta = '2' and num_pregunta = '$pregunta' and mes=gen.mes and regional = gen.regional)/ " .
					"(select count(num_pregunta)  " .
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NO ";
			} else {
				...
				$querydepartamento = "round((select count(num_respuesta)  " .
					...
					"where num_respuesta = '2' and num_pregunta = '$pregunta' and mes=gen.mes  and regional = gen.regional)/ " .
					"(select count(num_pregunta)  " .
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBNO,  " .
					"round((select count(num_respuesta)  " .
					"from seguimientopedidos.nps  " .
					"where num_respuesta = '3' and num_pregunta = '$pregunta' and mes=gen.mes and regional = gen.regional)/ " .
					"(select count(num_pregunta)  " .
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NOSEGURO,  " .
					"round((select count(num_respuesta)  " .
					"from seguimientopedidos.nps " .
					"where num_respuesta = '4' and num_pregunta = '$pregunta' and mes=gen.mes and regional = gen.regional)/ " .
					"(select count(num_pregunta)  " .
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBSI, " .
					"round((select count(num_respuesta) " .
					"from seguimientopedidos.nps  " .
					...
					"from seguimientopedidos.nps where regional = gen.regional and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as SI, " .
					"round(((select count(num_respuesta)  " .
					"from seguimientopedidos.nps where num_respuesta = '5' and num_pregunta = '$pregunta' and mes = gen.mes and regional = gen.regional) " .
					"-(select count(num_respuesta) " .
					"from seguimientopedidos.nps where num_respuesta = '1' and num_pregunta = '$pregunta' and mes = gen.mes and regional = gen.regional) " .
					"-(select count(num_respuesta) " .
					"from seguimientopedidos.nps where num_respuesta = '2' and num_pregunta = '$pregunta' and mes = gen.mes and regional = gen.regional) " .
					"-(select count(num_respuesta) " .
					"from seguimientopedidos.nps where num_respuesta = '3' and num_pregunta = '$pregunta' and mes = gen.mes and regional = gen.regional))/ " .
					"(select count(num_respuesta) " .
					"from seguimientopedidos.nps where num_pregunta = '4' and mes = gen.mes and regional = gen.regional)*100,2) as NPS ";
			}
			...
			if ($pregunta == "1" || $pregunta == "6" || $pregunta == "7" || $pregunta == "8") {
				...
				$valoresSemana = "(select count(num_respuesta) " .
					...
					"(select count(num_respuesta) from seguimientopedidos.nps  " .
					"where num_respuesta = '2' and num_pregunta = '$pregunta' and mes=gen.mes and semana = gen.semana ) as NO, " .
					"(select count(num_respuesta)  " .
					"from seguimientopedidos.nps where semana = gen.semana and mes=gen.mes and num_pregunta = '$pregunta' ) as TOTAL ";
			} else {
				...
				$valoresSemana = "(select count(num_respuesta) " .
					...
					"(select count(num_respuesta)  " .
					"from seguimientopedidos.nps where semana = gen.semana and num_pregunta = '$pregunta' and mes = gen.mes) as TOTAL ";
			}
			...
			if ($pregunta == "1" || $pregunta == "6" || $pregunta == "7" || $pregunta == "8") {
				...
				$querycontrato = "round((select count(contratista) " .
					...
					"from seguimientopedidos.nps where contratista = gen.contratista and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as SI,  " .
					...
					"(select count(contratista)  " .
					"from seguimientopedidos.nps where contratista = gen.contratista and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NO ";
			} else {
				...
				$querycontrato = "round((select count(contratista) " .
					...
					"where num_respuesta = '2' and num_pregunta = '$pregunta' and mes=gen.mes and contratista = gen.contratista)/ " .
					"(select count(contratista)  " .
					"from seguimientopedidos.nps where contratista = gen.contratista and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBNO,  " .
					"round((select count(contratista)  " .
					"from seguimientopedidos.nps  " .
					"where num_respuesta = '3' and num_pregunta = '$pregunta' and mes=gen.mes and contratista = gen.contratista)/ " .
					"(select count(contratista)  " .
					"from seguimientopedidos.nps where contratista = gen.contratista and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as NOSEGURO,  " .
					"round((select count(contratista)  " .
					"from seguimientopedidos.nps  " .
					"where num_respuesta = '4' and num_pregunta = '$pregunta' and mes=gen.mes and contratista = gen.contratista)/ " .
					"(select count(contratista)  " .
					"from seguimientopedidos.nps where contratista = gen.contratista and mes=gen.mes and num_pregunta = '$pregunta' limit 1 )*100, 2) as PROBSI,  " .
					"round((select count(contratista)  " .
					"from seguimientopedidos.nps  " .
	...
	private function listadoEstadosClick() {
		...
		$query = "select estado_id, count(pedido_id) total_estados " .
			"from seguimientopedidos.carga_click  " .
			"where fecha_cita BETWEEN ('$fecha 00:00:00') AND ('$fecha 23:59:59')  " .
			"$tipo_trabajo $uen " .
			"group by estado_id  " .
	...
	private function csvContingencias() {
		...
		$query = ("SELECT C.accion, C.ciudad, C.correo, C.macEntra, C.macSale, C.motivo, C.observacion,
					...
					C.horaContingenciaPortafolio, C.tipificacionPortafolio, C.observContingenciaPortafolio
	...
	private function insertData() {
		...
		//truncate table
		$query = "TRUNCATE TABLE seguimientopedidos.jornada_estados";
		$rr = $this->connseguimiento->query($query);
	...
	private function csvRegistros() {
		...
		$query = "select a.pedido,a.id_tecnico, " .
			...
			"a.proceso, a.producto,a.duracion,a.llamada_id, a.prueba_integrada,a.codigo_familiar, " .
			"a.smartplay,a.toip,a.inter,a.iptv,a.telev,a.totdm,a.plantilla  " .
			"from registros a " .
	...
	private function insertarCambioEquipo() {
		...
		if ($tecnologia == "DTH") {
			$sql = " INSERT INTO seguimientopedidos.cambio_equipos " .
	...
	private function expBrutal() {
		...
		$query = "select " .
			...
			"idLlamada " .
			"from seguimientopedidos.BrutalForce " .
	...
	private function pendiBrutal() {
		...
		$this->response($this->json(array($pedidospendiNO)), 201);
	}
		...
		if ($hora >= 10) {
			...
			if ($rstResumen->num_rows > 0) {
				...
				while ($row = $rstResumen->fetch_assoc()) {
					...
					$query = "UPDATE seguimientopedidos.BrutalForce SET " .
						" pedidobloqueado = '2' " .
	...
	private function gestionAsesorBrutal() {
		...
		$query = "select idgestion from seguimientopedidos.BrutalForce where PedidoDespacho = '$PedidoDespacho' ";
	...
	private function updateEnGestion() {
		...
		$query = ("
					SELECT id, engestion, pedido, observContingencia, observContingenciaPortafolio,
						...
						FROM seguimientopedidos.contingencias
	...
	private function datoscontingencias() {
		...
		$query = ("SELECT c.pedido, c.macEntra, c.macSale, c.logincontingencia, c.paquetes, c.ciudad, c.proceso, c.accion, c.tipoEquipo, c.remite, c.observacion, 
					...
					c.tipificacionPortafolio, c.enGestionPortafolio, c.fechaClickMarcaPortafolio, CASE WHEN (SELECT COUNT(*)
	...
	private function savecontingencia() {
		...
		$login = $params['login'];
		$login = $login['LOGIN'];
		$estadoActual = $datosguardar['estado'];
		$accion = $datosguardar['accion'];
		$ciudad = $datosguardar['ciudad'];
		$correo = $datosguardar['correo'];
		$macEntra = $datosguardar['macEntra'];
		$macSale = $datosguardar['macSale'];
		$motivo = $datosguardar['motivo'];
		$observacion = $datosguardar['observacion'];
		$pedido = $datosguardar['pedido'];
		$proceso = $datosguardar['proceso'];
		$remite = $datosguardar['remite'];
		$producto = $datosguardar['producto'];
		$tecnologia = $datosguardar['tecnologia'];
		$tipoEquipo = $datosguardar['tipoEquipo'];
		$uen = $datosguardar['uen'];
		$contrato = $datosguardar['contrato'];
		$perfil = $datosguardar['perfil'];
		$paquetes = $datosguardar['paquetes'];
		...
		if ($producto == "TV" && $accion == "Corregir portafolio") {
			...
			$grupo = "PORTAFOLIO";
		} else if ($producto == "Internet+ToIP" && $accion == "Corregir portafolio") {
			$grupo = "PORTAFOLIO";
		} else if ($producto == "TV" && $accion == "mesaOffline") {
			$grupo = "PORTAFOLIO";
		} else if ($producto == "Internet" && $accion == "mesaOffline") {
			$grupo = "PORTAFOLIO";
		} else if ($producto == "ToIP" && $accion == "mesaOffline") {
			$grupo = "PORTAFOLIO";
		...
		$sqlupdate = "INSERT INTO seguimientopedidos.contingencias " .
			"(accion,ciudad,correo,macEntra,macSale,motivo, " .
			"observacion,paquetes,pedido,proceso,producto, " .
			"remite,tecnologia,tipoEquipo,uen,contrato,perfil,grupo,logindepacho) " .
			" values ( " .
			" '$accion', '$ciudad', '$correo', '$macEntra', '$macSale', '$motivo', " .
	...
	private function CancelarContingencias() {
		...
		$query = "SELECT id FROM seguimientopedidos.contingencias WHERE pedido = '$pedido' AND engestion IS NULL OR pedido = '$pedido' AND engestion = '0'";
	...
	private function guardarpedidocontingencia() {
		...
		$pedido = $datosguardar['pedido'];
		$producto = $datosguardar['producto'];
		$observacionesconting = utf8_decode($datosguardar['observacionescontingencia']);
		$acepta = $datosguardar['acepta'];
		...
		$query = ("SELECT id
					...
					AND accion IN('Contingencia', 'Refresh', 'Cambio de equipo', 'Crear Espacio', 'crear cliente', 'Registros ToIP', 'mesaOffline', 'Cambio EID')
				");
	...
	private function marca() {
		...
		$query = "SELECT id, logincontingencia " .
			"FROM seguimientopedidos.contingencias where engestion = '1'  " .
			"and finalizado is null " .
	...
	private function guardarPedidoContingenciaPortafolio() {
		...
		$aceptaPortafolio = $datosguardar['aceptaPortafolio'];
		$ingresoClick = $datosguardar['ingresoClick'];
		...
		if ($aceptaPortafolio == true) {
			$aceptaPortafolio = 'Acepta';
		} else if ($aceptaPortafolio == "") {
	...
	private function guardarEscalar() {
		...
		$query = "select idgestion, estado from seguimientopedidos.BrutalForce where PedidoDespacho = '$PedidoDespacho' ";
	...
	private function desbloquear() {
		...
		$query = "select idgestion id from seguimientopedidos.BrutalForce where PedidoDespacho = '$PedidoDespacho' ";
	...
	private function gestionBorrar() {
		...
		if ($rst->num_rows > 0) {
			...
			$sqlDelete = "DELETE FROM seguimientopedidos.BrutalForce WHERE idgestion='$id' ";
	...
	private function gestiodespachoBrutal() {
		...
		$query = "select PedidoDespacho from seguimientopedidos.BrutalForce where PedidoDespacho = '$pedido' ";
		$rst = $this->connseguimiento->query($query);
		...
		if ($rst->num_rows > 0) {
			...
			$sql = " INSERT INTO seguimientopedidos.BrutalForce " .
	...
	private function GuardarPedidoEncuesta() {
		...
		$sql = " INSERT INTO seguimientopedidos.doloresClientes " .
	...
	private function guardarPlan() {
		...
		$sql = " INSERT INTO seguimientopedidos.npsPlanTrabajo ( " .
			" responsable, " .
			" regional, " .
	...
	private function DemePedidoPendiInsta() {
		...
		$query = "SELECT id, Pedido,	Interfaz, Tipo_solicitud_orig, Departamento, SubZona, Concepto_ATC,	Detalle_concepto_Oracle, " .
			...
			"FROM seguimientopedidos.gestion_pendientes " .
	...
	private function guardarTurnos() {
		...
		for ($i = 0; $i < $total; $i++) {
			...
			$insert = " INSERT INTO seguimientopedidos.turnosSeguimiento " .
